...
function hasForbiddenReflectionPhrase(value: string): boolean {
  ...
  return FORBIDDEN_REFLECTION_PHRASES.some((phrase) =>
...
function selectCurrentPeriodId(
  ...
  const current = rows.find(
    (row) => row.period_start <= day && row.period_end >= day,
  );
...
export default function ReflectionsScreen() {
  ...
  const [weekReflections, setWeekReflections] = useState<
    Awaited<ReturnType<typeof fetchRecentReflectionsByType>>
  >([]);
  const [monthReflections, setMonthReflections] = useState<
    Awaited<ReturnType<typeof fetchRecentReflectionsByType>>
  >([]);
  const [weekHasMore, setWeekHasMore] = useState(false);
  const [monthHasMore, setMonthHasMore] = useState(false);
  const [loadingMorePeriod, setLoadingMorePeriod] = useState<PeriodType | null>(
    null,
  );
  ...
  const activeRows =
    activePeriod === "week" ? weekReflections : monthReflections;
  ...
  const selectorSelectedId =
    selectorPeriod === "week" ? selectedWeekId : selectedMonthId;
  const selectorHasMore =
    selectorPeriod === "week" ? weekHasMore : monthHasMore;
  const highlights = activeReflection
    ? parseJsonStringArray(activeReflection.highlights_json)
    : [];
  ...
  const headerActionLabel =
    activePeriod === "week" ? "Kies week" : "Kies maand";
  ...
  const selectorSections = useMemo<ArchiveGroupedListSection[]>(() => {
    ...
    for (const row of selectorRows) {
      ...
      const weekNumber = Number.isNaN(weekDate.getTime())
        ? "--"
        : String(getIsoWeekNumber(weekDate));
      const item = {
        key: row.id,
        leftTop: selectorPeriod === "week" ? "week" : monthLabels.top,
        leftBottom: selectorPeriod === "week" ? weekNumber : monthLabels.bottom,
        snippet:
          summary.length > 0 ? summary : "Nog geen samenvatting beschikbaar.",
  ...
  const loadMoreSelector = useCallback(async () => {
    ...
    try {
      ...
      const merged = [
        ...currentRows,
        ...nextRows.filter((row) => !existing.has(row.id)),
      ];
      if (selectorPeriod === "week") {
            ...
            {!loading ? (
              <ThemedView style={styles.readingCanvas}>
                        ...
                        <ThemedView style={styles.highlightList}>
                          ...
                          {cleanHighlights.map((item, index) => {
                            ...
                            return (
                              <ThemedView
                                key={`${item}-${index}`}
                                style={styles.highlightEditorialRow}
                              >
                                ...
                                <ThemedView style={styles.highlightTrackCol}>
                                  <ThemedView
                                    style={[
                                      ...
                                      {
                                        borderColor: `${palette.primary}CC`,
                                  ...
                                  {index < cleanHighlights.length - 1 ? (
                                    <ThemedView
                                      style={[
                                        styles.highlightTrackLine,
                                        {
                                          backgroundColor: `${palette.separator}88`,
                                        },
                                      ]}
                                    />
                                  ) : null}
                                ...
                                <ThemedView
                                  style={styles.highlightEditorialContent}
                                >
                                  <ThemedText
                                    type="bodySecondary"
                                    style={[
                        ...
                        <ThemedView style={styles.highlightList}>
                          ...
                          <ThemedView style={styles.reflectionPointsCard}>
                            {cleanReflectionPoints.map((item, index) => (
                              <ThemedView
                                key={`${item}-${index}`}
                                style={styles.reflectionPointItem}
                              >
                                <ThemedText
                                  type="body"
                                  style={[
                                    styles.reflectionPointText,
                                    { color: palette.text },
                                  ]}
                                >
                                  {item}
                                ...
                                />
                                {index < cleanReflectionPoints.length - 1 ? (
                                  <ThemedView
                                    style={styles.reflectionPointGap}
                                  />
                                ) : null}
                              </ThemedView>
                            ))}
                ...
                <ThemedView style={styles.bottomActionWrap}>
                  <SecondaryButton
                    label={
                      isProcessing ? "Hergenereert..." : bottomGenerateLabel
                    }
